...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	MaterialTechniqueContext context;
	if (isSkinned)
		context = m_GeneratorTechniqueContexts[(int)ShadowGeneratorType::Skinned];
	else
		context = m_GeneratorTechniqueContexts[(int)ShadowGeneratorType::Static];
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//		*nearZ>0.1f
	//		*farZ>500.f
	float viewWidth = m_ShadowmapScaleMultiplier * 100.0f * (m_GameContext.windowWidth / m_GameContext.windowHeight);
	float viewHeight = m_ShadowmapScaleMultiplier * 100.0f;
	float nearZ = 0.1f;
	float farZ = 500.0f;
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	if (isSkinned) m_pShadowMapGenerator->SetTechnique((int)ShadowGeneratorType::Skinned);
	else m_pShadowMapGenerator->SetTechnique((int)ShadowGeneratorType::Static);
	...
	for (const auto& subMesh : pMeshFilter->GetMeshes())
	{
		...
		sceneContext.d3dContext.pDeviceContext->IASetVertexBuffers(0, 1, &vertexBufferData.pVertexBuffer, &vertexBufferData.VertexStride,
			&offset);
...
void ShadowMapRenderer::SetShadowmapScaleMultiplier(float value)
{
	m_ShadowmapScaleMultiplier = value;
	/*delete m_pShadowRenderTarget;
	...
	desc.enableColorBuffer = false;
	desc.enableDepthSRV = true;
	desc.width = UINT(m_ShadowmapScaleMultiplier * m_GameContext.windowWidth);
	desc.height = UINT(m_ShadowmapScaleMultiplier * m_GameContext.windowHeight);
	m_pShadowRenderTarget->Create(desc);*/
}
