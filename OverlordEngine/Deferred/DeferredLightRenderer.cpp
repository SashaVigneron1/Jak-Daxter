...
void DeferredLightRenderer::Initialize(const D3D11Context& d3dContext)
{
	...
	const auto inputLayout = m_pVolumetricLightMaterial->GetTechniqueContext().inputLayoutID;
	...
	m_pSphereVB = m_pSphereMesh->GetVertexBufferData(inputLayout).pVertexBuffer;
	...
	auto& vbData = m_pConeMesh->GetVertexBufferData(inputLayout);
...
void DeferredLightRenderer::DrawVolumetricLight(const SceneContext& sceneContext, const Light& light) const
{
	...
	auto mvp = world * XMLoadFloat4x4(&sceneContext.pCamera->GetViewProjection());

	m_pVolumetricLightMaterial->SetVariable_Matrix(L"gWorldViewProjection", reinterpret_cast<float*>(&mvp));
