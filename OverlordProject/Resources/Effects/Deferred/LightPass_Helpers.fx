...
LightingResult DoSpotLighting(Light light, Material mat, float3 V, float3 N, float3 P)
{
	...
	float attenuation = DoAttenuation(light, distance);
	float coneFallof = DoSpotCone(light, L);

	result.Diffuse = DoDiffuse(light, L, N) * attenuation * coneFallof * light.Intensity;
	result.Specular = DoSpecular(light, mat, V, L, N) * attenuation * coneFallof * light.Intensity;
