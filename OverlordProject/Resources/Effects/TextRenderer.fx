...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, int channel)
{
	...
	GS_DATA gData = (GS_DATA)0;
	
	//Fill in all the fields
	gData.Position = mul(float4(pos, 1.0f), gTransform);
	gData.Color = col;
	gData.TexCoord = texCoord;
	gData.Channel = channel;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	float uvWidth = (vertex[0].CharSize.x / gTextureSize.x);
	float uvHeight = (vertex[0].CharSize.y / gTextureSize.y);
	
	//1. Vertex Left-Top
	CreateVertex(triStream, position, vertex[0].Color, texCoord, vertex[0].Channel);

	//2. Vertex Right-Top
	position.x = vertex[0].Position.x + width;
	position.y = vertex[0].Position.y;
	texCoord.x = vertex[0].TexCoord.x + uvWidth;
	...
	position.y = vertex[0].Position.y + height;
	texCoord.x = vertex[0].TexCoord.x;
	texCoord.y = vertex[0].TexCoord.y + uvHeight;
