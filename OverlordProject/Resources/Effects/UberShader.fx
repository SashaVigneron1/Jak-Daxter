...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    if (gUseTextureSpecularIntensity) specularIntensity = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r;
    
    float3 halfVector = -normalize(viewDirection + gLightDirection);
    float specularStrength = saturate(dot(halfVector, normal));
    
    float3 specularColor = gColorSpecular * pow(specularStrength, gShininess) * specularIntensity;
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specularStrength = saturate(dot(reflected, viewDirection));
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	if (gUseTextureNormal)
	{
		...
		// Normal Calculations
		float4 normalSample = gTextureNormal.Sample(gTextureSampler, texCoord);
		normalSample = (normalSample * 2.0f) - float4(1.0f, 1.0f, 1.0f, 1.0f);
		float3 binormal = cross(normal, tangent);
		
		if (gFlipGreenChannel) binormal *= -1;
		
		float3x3 tangentSpaceAxis = float3x3(tangent, binormal, normal);
		newNormal = normalize(mul(normalSample.xyz, tangentSpaceAxis));
	}
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	if (gUseEnvironmentMapping)
	{
		...
		environmentColor = reflection + refraction;
	}
