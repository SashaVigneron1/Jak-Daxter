...
VS_OUTPUT VS(VS_INPUT input){
	...
	for (int i = 0; i < 4; ++i)
    {
        ...
        transformedPosition += mul(float4(input.pos, 1.0) * input.blendWeights[i], (float4x4) gBones[boneIndex]);
		transformedNormal += mul(input.normal * input.blendWeights[i], (float3x3) gBones[boneIndex]);
    }
