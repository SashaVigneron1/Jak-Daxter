...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float3 centerPoint, topPoint, leftPoint, rightPoint, spikeNormal;

	//Step 1. Calculate CENTER_POINT
	leftPoint = vertices[0].Position.xyz;
	topPoint = vertices[1].Position.xyz;
	rightPoint = vertices[2].Position.xyz;
	
	centerPoint =  (leftPoint + rightPoint + topPoint) / 3;
	
	//Step 2. Calculate Face Normal (Original Triangle)
	float3 e0 = topPoint - leftPoint;
    float3 e1 = rightPoint - leftPoint;
    spikeNormal = normalize(cross(e1, e0));
	
	//Step 3. Offset CENTER_POINT (use gSpikeLength)
	centerPoint += spikeNormal * gSpikeLength;
	
	//Step 4 + 5. Calculate Individual Face Normals (Cross Product of Face Edges) & Create Vertices for every face

    //FACE 1
    e0 = centerPoint - topPoint;
    e1 = topPoint - leftPoint;
    float3 faceNormal = normalize(cross(e0, e1));
    CreateVertex(triStream, topPoint, faceNormal, (float2)0);
    CreateVertex(triStream, centerPoint, faceNormal, (float2)0);
    CreateVertex(triStream, leftPoint, faceNormal, (float2)0);

    //Restart Strip! >> We want to start a new triangle (= interrupt trianglestrip)
    triStream.RestartStrip();

    //FACE 2
    e0 = centerPoint - rightPoint;
    e1 = topPoint - rightPoint;
    faceNormal = normalize(cross(e1, e0));
    CreateVertex(triStream, rightPoint, faceNormal, (float2)0);
    CreateVertex(triStream, centerPoint, faceNormal, (float2)0);
    CreateVertex(triStream, topPoint, faceNormal, (float2)0);
    ...
    //Face 3
	e0 = centerPoint - leftPoint;
    e1 = rightPoint - leftPoint;
    faceNormal = normalize(cross(e1, e0));
    CreateVertex(triStream, leftPoint, faceNormal, (float2)0);
    CreateVertex(triStream, centerPoint, faceNormal, (float2)0);
    CreateVertex(triStream, rightPoint, faceNormal, (float2)0);
